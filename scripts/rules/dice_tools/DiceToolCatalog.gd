...
const TOOL_SOURCE_BASIC := &"basic"
const TOOL_SOURCE_MID := &"mid"
const TOOL_SOURCE_LATE := &"late"
const TOOL_SOURCE_SPECIAL := &"special"

const NORMAL_SHOP_MID_SOURCE_INDICES := [
	24, 30, 31, 35, 42, 49, 52, 56, 57, 73, 79, 81, 83, 88, 89, 93, 95, 96, 98, 103, 104, 106, 110, 116, 118, 119, 129, 139, 145,
]

const NORMAL_SHOP_NEVER_SOURCE_INDICES := [
	26, 51, 54, 59, 61, 66, 67, 71, 72, 85, 86, 92, 99, 109, 123, 126, 133, 137, 138, 142, 144, 146, 147, 148, 149, 150,
]

const MEGA_PACK_EPIC_LOW_WEIGHT_SOURCE_INDICES := [
	51, 61, 72, 92, 99, 109, 126, 133, 144,
]
...
static func get_all_defs() -> Array[DiceToolDef]:
	return [
		...
		_make_def(TOOL_EMPTY_SLOT_XMULT, "空槽棱镜", "Joker Stencil", 17, &"rare", "结算时，终倍率 ×N。N = 空骰具槽数量 + 1；负载骰具不占用槽位。", &"on_score", [&"xmult", &"slot"]),
		_make_def(TOOL_SHORT_STRAIGHT_RULE, "四指顺规", "Four Fingers", 18, &"rare", "顺子判定只需要 4 个连续有效点数。", &"rule_modifier", [&"straight"]),
		_make_def(TOOL_UNSCORED_RETRIGGER, "留场复诵器", "Mime", 19, &"rare", "所有未结算留场骰面的留场触发效果额外执行 1 次。", &"after_unscored_stay", [&"retrigger", &"stay"]),
		_make_def(TOOL_CREDIT_DEBT, "负债额度卡", "Credit Card", 20, &"common", "金币最低可以降到 -20。", &"economy_rule", [&"coins"]),
		_make_def(TOOL_SELL_VALUE_DAGGER, "祭价短刃", "Ceremonial Dagger", 21, &"rare", "选择战斗时，摧毁右侧相邻骰具；本骰具永久获得倍率：被摧毁骰具卖价 ×2。", &"on_battle_start", [&"mult", &"destroy"], [], [&"mult_bonus"]),
		...
		_make_def(TOOL_SIX_ROUND_XMULT, "六回合忠诚器", "Loyalty Card", 25, &"rare", "每第 6 次结算回合，终倍率 ×4。", &"on_score", [&"xmult", &"counter"], [], [&"scored_round_counter"]),
		_make_def(TOOL_SIX_FORGE_GENERATOR, "六点铸件球", "8 Ball", 26, &"uncommon", "每个被结算的 6 点面有 1/4 概率生成 1 个随机铸骰件；需要 1 个空道具槽位。", &"on_scored_face", [&"generate", &"six"]),
		_make_def(TOOL_RANDOM_MULT, "乱码倍率器", "Misprint", 27, &"common", "结算时，随机 +0 到 +23 倍率。", &"on_score", [&"mult", &"random"]),
		_make_def(TOOL_FINAL_ROUND_RETRIGGER, "黄昏复触器", "Dusk", 28, &"rare", "在本场最后一回合中，所有被结算骰面额外触发 1 次。", &"on_score", [&"retrigger"]),
		_make_def(TOOL_LOWEST_UNSCORED_MULT, "留场低拳", "Raised Fist", 29, &"common", "将未结算留场骰面中的最低有效点数 ×2 加入倍率；没有有效点数则不触发。", &"on_score", [&"mult", &"stay"]),
		_make_def(TOOL_FREE_SHOP_REROLL, "免费刷新器", "Chaos the Clown", 30, &"uncommon", "每个骰商铺获得 1 次免费刷新。", &"on_shop_open", [&"shop"]),
		_make_def(TOOL_FIBONACCI_MULT, "斐波那契增幅器", "Fibonacci", 31, &"uncommon", "每个被结算的 1 / 2 / 3 / 5 / 8 点面 +8 倍率。", &"on_scored_face", [&"mult", &"pip"]),
		_make_def(TOOL_STAY_ORNAMENT_XMULT, "留场钢核", "Steel Joker", 32, &"rare", "完整出战骰组中，每 5 个留场面饰骰面，使本骰具终倍率 +1；本骰具基础终倍率为 ×1。", &"on_score", [&"xmult", &"stay"]),
		...
		_make_def(TOOL_NO_REROLL_INCOME, "延迟收益器", "Delayed Gratification", 35, &"uncommon", "回合结束时，若本回合没有使用重投，则每个剩余重投次数获得 2 金币。", &"round_end", [&"coins", &"reroll"]),
		_make_def(TOOL_LOW_MID_RETRIGGER, "低中点破解器", "Hack", 36, &"rare", "重新触发每个被结算的 2 / 3 / 4 / 5 点面。", &"on_scored_face", [&"retrigger", &"pip"]),
		_make_def(TOOL_ALL_FACES_HIGH_FOR_TOOLS, "高点拟像器", "Pareidolia", 37, &"rare", "所有非石质骰面在骰具条件判断中都视为高点面；实际点数不变。", &"rule_modifier", [&"high"]),
		...
		_make_def(TOOL_HIGH_INCOME_CHANCE, "高点名片", "Business Card", 42, &"uncommon", "每个被结算高点面有 1/2 概率获得 2 金币；高点面为有效点数 5 / 6 / 7 / 8。", &"on_scored_face", [&"coins", &"high"]),
		...
		_make_def(TOOL_SPACE_COMBO_UPGRADE, "星隙升格器", "Space Joker", 45, &"rare", "结算时有 1/4 概率使本回合主骰型等级 +1。", &"on_score", [&"combo_level", &"random"]),
		_make_def(TOOL_GROWING_SELL_VALUE, "增值蛋壳", "Egg", 46, &"common", "每回合结束时，本骰具卖价 +3 金币。", &"round_end", [&"sell_value"], [], [&"sell_value"]),
		_make_def(TOOL_ROUNDS_FOR_NO_REROLL, "窃回合者", "Burglar", 47, &"rare", "选择战斗时，本场战斗结算回合数 +3，且本场战斗重投次数上限设为 0。", &"on_battle_start", [&"rounds", &"reroll"]),
		_make_def(TOOL_UNSCORED_LOW_HIGH_XMULT, "留场黑板", "Blackboard", 48, &"rare", "若本回合存在至少 1 个有效未结算留场骰面，且这些有效未结算留场骰面全部为低点面或全部为高点面，则终倍率 ×3。", &"on_score", [&"xmult", &"stay"]),
		_make_def(TOOL_STRAIGHT_RUNNER_CHIPS, "顺子跑者", "Runner", 49, &"uncommon", "若本回合满足顺子结构，本骰具永久获得 +15 基础战力；结算时提供其当前累计的基础战力。", &"on_score", [&"chips", &"straight", &"counter"], [], [&"chips_bonus"]),
		...
		_make_def(TOOL_SINGLE_FACE_BLUEPRINT, "单面样本", "DNA", 51, &"epic", "每场战斗第一回合只结算 1 个骰面时，进入待处理复制覆盖状态，选择现有目标骰面后覆盖点数、面饰、印记。", &"on_first_round_single_score", [&"copy"], [], []),
		_make_def(TOOL_SPLASH_SCORING, "溅射计分", "Splash", 52, &"uncommon", "本回合被结算但未参与主骰型结构的骰面，其面饰额外触发 1 次；不重复点数总和。", &"on_score", [&"ornament", &"retrigger"]),
		_make_def(TOOL_LEFTOVER_CHIPS, "余面筹码", "Blue Joker", 53, &"common", "完整出战骰组中，每个本回合未被结算的投出骰面位，+2 基础战力。", &"on_score", [&"chips", &"unscored"]),
		_make_def(TOOL_SINGLE_SIX_REFORGE, "六感重铸", "Sixth Sense", 54, &"rare", "每场战斗第一回合只结算 1 个 6 点面时，结算后重置该物理骰面并生成 1 个随机铸骰件；需要空道具槽位。", &"on_first_round_single_score", [&"generate", &"six", &"reset"]),
		_make_def(TOOL_STAR_COUNTER, "星群计数器", "Constellation", 55, &"rare", "本局每使用 3 个主骰型升级件，本骰具终倍率 +1；计分时提供整数终倍率。", &"on_score", [&"xmult", &"combo_upgrade"], [], [&"combo_upgrade_used_count"]),
		...
		_make_def(TOOL_HIGH_REROLL_BOUNTY, "高点悬赏", "Faceless Joker", 57, &"uncommon", "若一次重投中选择了 3 个或更多高点面，获得 5 金币。", &"before_reroll", [&"coins", &"high", &"reroll"]),
		...
		_make_def(TOOL_STRAIGHT_SIX_SUPPLY, "顺六补给", "Superposition", 59, &"uncommon", "若本回合主骰型为顺子，且被结算有效骰面中包含 6 点面，生成 1 个随机铸骰件；需要空道具槽位。", &"on_score", [&"generate", &"straight", &"six"]),
		...
		_make_def(TOOL_UNSTABLE_X3, "不稳定三倍器", "Cavendish", 61, &"epic", "计分时终倍率 ×3；战斗结束时有 1/1000 概率自毁。", &"on_score_and_battle_end", [&"xmult", &"destroy"]),
		_make_def(TOOL_REPEAT_COMBO_X3, "复现三倍器", "Card Sharp", 62, &"rare", "若本回合主骰型在本场战斗中此前已经结算过，终倍率 ×3；结算后记录本回合主骰型。", &"on_score", [&"xmult", &"combo"], [], [&"seen_combo_ids_this_battle"]),
		_make_def(TOOL_SKIP_PACK_MULT, "跳包倍率器", "Red Card", 63, &"common", "每跳过 1 个骰包，本骰具永久获得 +3 倍率。", &"on_pack_skipped", [&"mult", &"pack"], [], [&"skipped_pack_count"]),
		_make_def(TOOL_MAD_GROWTH, "狂乱增长器", "Madness", 64, &"rare", "每选择 1 次非 Boss 战斗，本骰具狂乱计数 +1，并随机摧毁 1 个其他已安装骰具；每 2 点计数使终倍率 +1。", &"on_battle_start", [&"xmult", &"destroy"], [], [&"madness_charge"]),
		...
		_make_def(TOOL_STRAIGHT_FORGE_SUPPLY, "顺子铸件供给", "Séance", 66, &"rare", "若本回合主骰型为顺子，生成 1 个随机进阶铸骰件；需要空道具槽位。", &"on_score", [&"generate", &"straight"]),
		_make_def(TOOL_COMMON_TOOL_SUPPLY, "普通骰具补给", "Riff-Raff", 67, &"rare", "选择战斗时，生成最多 2 个随机普通骰具道具，进入道具槽位。", &"on_battle_start", [&"generate", &"dice_tool"]),
		_make_def(TOOL_ORNAMENT_VAMPIRE, "面饰吞噬者", "Vampire", 68, &"rare", "每个被结算且带有面饰的骰面，会被本骰具吞噬面饰；每吞噬 10 个面饰，本骰具终倍率 +1。", &"on_score", [&"xmult", &"ornament"], [], [&"absorbed_ornament_count"]),
		_make_def(TOOL_SHORTCUT_STRAIGHT, "捷径顺规", "Shortcut", 69, &"rare", "顺子判定允许缺 1 个中间点数。", &"rule_modifier", [&"straight"]),
		_make_def(TOOL_COPY_HOLOGRAM, "复写增幅器", "Hologram", 70, &"rare", "每当 1 个现有骰面被复制覆盖，本骰具复制计数 +1；每 2 点复制计数使终倍率 +1。", &"on_face_copied", [&"xmult", &"copy"], [], [&"copied_face_count"]),
		_make_def(TOOL_POOR_FORGE_SUPPLY, "低资铸件供给", "Vagabond", 71, &"rare", "若结算后当前金币 ≤4，生成 1 个随机铸骰件；需要空道具槽位。", &"on_score", [&"generate", &"coins"]),
		_make_def(TOOL_SIX_STAY_KING, "六点留王", "Baron", 72, &"epic", "未结算留场的每个 6 点面，使终倍率 ×2。", &"on_score", [&"xmult", &"stay", &"six"]),
		_make_def(TOOL_FIVE_FACE_INCOME, "五点存款", "Cloud 9", 73, &"uncommon", "回合结束时，完整出战骰组中每个 5 点骰面获得 1 金币。", &"round_end", [&"coins", &"pip"]),
		_make_def(TOOL_ROCKET_INCOME, "火箭收益器", "Rocket", 74, &"rare", "回合结束时获得 N 金币，初始 N = 1；每击败 1 场 Boss 战斗，N 永久 +2。", &"round_end_and_boss_win", [&"coins"], [], [&"rocket_income"]),
		...
		_make_def(TOOL_HIGH_TO_GOLD, "高点镀金器", "Midas Mask", 76, &"rare", "所有被结算的高点面，在结算后变为金辉面饰。", &"on_score", [&"ornament", &"high"]),
		_make_def(TOOL_BOSS_RULE_BREAKER, "Boss 破规器", "Luchador", 77, &"rare", "出售此骰具时，若当前处于 Boss 战斗，禁用当前 Boss 规则。", &"on_tool_sold", [&"boss"]),
		_make_def(TOOL_FIRST_HIGH_X2, "首高摄影机", "Photograph", 78, &"rare", "本回合第一个被结算的高点面，触发终倍率 ×2。", &"on_score", [&"xmult", &"high"]),
		_make_def(TOOL_SELL_VALUE_GROWTH, "礼券增值器", "Gift Card", 79, &"uncommon", "回合结束时，每个已安装骰具和每个道具槽位中的道具卖价 +1 金币。", &"round_end", [&"sell_value"]),
		_make_def(TOOL_MAX_SCORE_DECAY, "龟甲扩容器", "Turtle Bean", 80, &"rare", "每回合最大可结算骰面数 +N，初始 N = 5；每回合结束后 N -1，降到 0 时自毁。", &"battle_rule_and_round_end", [&"score_limit", &"decay"], [], [&"bean_bonus"]),
		...
		_make_def(TOOL_MAIL_PIP_REBATE, "指点返利", "Mail-In Rebate", 83, &"uncommon", "每重投 1 个当前指定点数的骰面，获得 5 金币；指定点数在回合结束时重新随机。", &"before_reroll", [&"coins", &"reroll"], [], [&"rebate_pip"]),
		_make_def(TOOL_INTEREST_BOOSTER, "月息放大器", "To the Moon", 84, &"rare", "回合结束时，每拥有 5 金币，额外获得 1 金币。", &"round_end", [&"coins"]),
		_make_def(TOOL_PACK_HALLUCINATION, "开包幻觉", "Hallucination", 85, &"uncommon", "打开任意骰包时，有 1/2 概率生成 1 个随机铸骰件；需要空道具槽位。", &"on_pack_opened", [&"generate", &"pack"]),
		_make_def(TOOL_FORGE_ITEM_ORACLE, "铸件占卜器", "Fortune Teller", 86, &"uncommon", "本局每使用 1 个铸骰件，+1 倍率。", &"on_score", [&"mult", &"forge_item"]),
		_make_def(TOOL_SCORE_SLOT_PLUS_ONE, "结算扩位器", "Juggler", 87, &"rare", "每回合最大可结算骰面数 +1。", &"battle_rule", [&"score_limit"]),
		_make_def(TOOL_REROLL_PLUS_ONE, "追加重投器", "Drunkard", 88, &"uncommon", "每回合重投次数 +1。", &"battle_rule", [&"reroll"]),
		_make_def(TOOL_STONE_FACE_CHIPS, "石面筹码器", "Stone Joker", 89, &"uncommon", "完整出战骰组中每个石质面饰骰面，+25 基础战力。", &"on_score", [&"chips", &"stone"]),
		_make_def(TOOL_ROUND_GOLD_INCOME, "固定金币器", "Golden Joker", 90, &"rare", "回合结束时获得 4 金币。", &"round_end", [&"coins"]),
		_make_def(TOOL_LUCKY_CAT_INTEGER, "幸运猫", "Lucky Cat", 91, &"rare", "每当幸运面饰成功触发 1 次，幸运计数 +1；每 2 点幸运计数使终倍率 +1。", &"on_lucky_success", [&"xmult", &"lucky"], [], [&"lucky_success_count"]),
		_make_def(TOOL_UNCOMMON_TEAM_XMULT, "罕见联队", "Baseball Card", 92, &"epic", "每有 2 个已安装的罕见骰具，终倍率 ×2。", &"on_score", [&"xmult", &"rarity"]),
		_make_def(TOOL_COIN_CHIPS, "金币筹码器", "Bull", 93, &"uncommon", "每拥有 1 金币，+2 基础战力。", &"on_score", [&"chips", &"coins"]),
		_make_def(TOOL_DOUBLE_REWARD_TAG, "双奖汽水", "Diet Cola", 94, &"rare", "出售此骰具时，获得 1 个双倍奖励标记。", &"on_tool_sold", [&"reward"]),
		...
		_make_def(TOOL_SHOP_REROLL_MULT, "刷新倍率器", "Flash Card", 96, &"uncommon", "骰商铺每刷新 1 次，本骰具永久获得 +2 倍率。", &"on_shop_rerolled", [&"mult", &"shop"], [], [&"shop_reroll_count"]),
		...
		_make_def(TOOL_ANCIENT_POINT_CLASS, "古老点域器", "Ancient Joker", 99, &"epic", "每回合指定奇数、偶数、低点、高点之一；被结算且符合当前分类的每个有效骰面，终倍率 ×2。", &"on_score_and_round_end", [&"xmult", &"pip_class"], [], [&"ancient_class"]),
		_make_def(TOOL_REROLL_DECAY_X2, "拉面衰减器", "Ramen", 100, &"rare", "初始终倍率 ×2；本局每重投 10 个骰面，本骰具终倍率降低 1，最低为 ×1。", &"on_score_and_reroll", [&"xmult", &"reroll"], [], [&"rerolled_face_count_for_ramen"]),
		...
		_make_def(TOOL_SELTZER_RETRIGGER, "气泡重触器", "Seltzer", 102, &"rare", "若剩余重触回合数大于 0，当前回合所有被结算骰面额外触发 1 次；结算结束后计数 -1，降到 0 时自毁。", &"on_score_and_round_end", [&"retrigger", &"decay"], [], [&"remaining_retrigger_rounds"]),
		...
		_make_def(TOOL_HIGH_SMILE_MULT, "高点笑脸", "Smiley Face", 104, &"uncommon", "每个被结算高点面 +5 倍率。高点面为有效点数 5 / 6 / 7 / 8。", &"on_scored_face", [&"mult", &"high"]),
		...
		_make_def(TOOL_GOLD_ORNAMENT_INCOME, "金辉票根", "Golden Ticket", 106, &"uncommon", "每个被结算且带有金辉面饰的骰面，获得 4 金币。", &"on_scored_face", [&"coins", &"ornament"]),
		_make_def(TOOL_BONE_SAFETY, "保底骨架", "Mr. Bones", 107, &"rare", "若本场累计战力未达标但至少达到目标战力四分之一，则避免本次失败并自毁。", &"before_defeat", [&"safety"]),
		_make_def(TOOL_FINAL_ROUND_ACROBAT, "终回合杂技", "Acrobat", 108, &"rare", "若当前回合是本场战斗最后一回合，终倍率 ×3。", &"on_score", [&"xmult", &"final_round"]),
		_make_def(TOOL_HIGH_FACE_RETRIGGER, "高点复演", "Sock and Buskin", 109, &"epic", "所有被结算高点面额外触发 1 次。", &"on_scored_face", [&"retrigger", &"high"]),
		_make_def(TOOL_SELL_VALUE_SWORD, "卖价剑客", "Swashbuckler", 110, &"uncommon", "结算时，获得等同其他所有已安装骰具卖价总和的倍率。", &"on_score", [&"mult", &"sell_value"]),
		_make_def(TOOL_TROUBADOUR_SLOTS, "吟游扩位器", "Troubadour", 111, &"rare", "每回合最大可结算骰面数 +2；每场战斗可结算回合数 -1，最低为 1。", &"battle_rule", [&"score_limit", &"rounds"]),
		_make_def(TOOL_MARKED_TEMP_FACE, "印记临面证", "Certificate", 112, &"rare", "回合开始时，向当前投骰池加入 1 个当前回合临时骰面；该骰面有随机点数和随机印记，回合结束后移除。", &"round_start", [&"temporary_face", &"mark"]),
		_make_def(TOOL_FACT_TOLERANCE, "宽容判定器", "Smeared Joker", 113, &"rare", "全奇、全偶、全低、全高四个事实判定允许 1 个有效被结算骰面不符合条件。", &"rule_modifier", [&"fact"]),
		_make_def(TOOL_SKIP_THROWBACK, "跳战回响", "Throwback", 114, &"rare", "本局每跳过 2 个战斗节点，本骰具终倍率 +1；结算时按累计加成提供整数终倍率。", &"on_score", [&"xmult", &"skip"]),
		_make_def(TOOL_FIRST_FACE_RETRIGGER, "首面重演器", "Hanging Chad", 115, &"rare", "当前回合第一个被结算骰面额外触发 2 次。", &"on_scored_face", [&"retrigger"]),
		_make_def(TOOL_EVEN_COIN_GEM, "偶数粗宝石", "Rough Gem", 116, &"uncommon", "每个被结算偶数面获得 1 金币。", &"on_scored_face", [&"coins", &"even"]),
		_make_def(TOOL_ODD_BLOODSTONE, "奇数血石", "Bloodstone", 117, &"rare", "每个被结算奇数面有 1/2 概率提供终倍率 ×2；概率可被概率规则修正。", &"on_scored_face", [&"xmult", &"odd", &"chance"]),
		...
		_make_def(TOOL_BURST_BREAK_GLASS, "爆裂玻璃", "Glass Joker", 120, &"rare", "每有 1 个爆裂面饰实际破碎，本骰具终倍率 +1；结算时按累计加成提供整数终倍率。", &"on_burst_break_and_score", [&"xmult", &"burst"], [], [&"xmult_bonus"]),
		_make_def(TOOL_DUPLICATE_SHOWMAN, "重复艺人", "Showman", 121, &"rare", "骰具、铸骰件、主骰型升级件可以重复出现；只影响后续生成规则。", &"generation_rule", [&"duplicate"]),
		_make_def(TOOL_FOUR_FACT_POT, "四类花盆", "Flower Pot", 122, &"rare", "若当前回合至少 4 个有效被结算骰面同时存在奇数、偶数、低点、高点四类，终倍率 ×3。", &"on_score", [&"xmult", &"pip_class"]),
		_make_def(TOOL_RIGHT_COPY_BLUEPRINT, "右侧蓝图", "Blueprint", 123, &"legendary", "复制右侧第 1 个可复制骰具的能力；复制类骰具不会被复制。", &"copy", [&"copy"]),
		...
		_make_def(TOOL_MERRY_REROLLER, "快乐重投手", "Merry Andy", 125, &"rare", "每回合重投次数 +3；每回合最大可结算骰面数 -1，最低为 1。", &"battle_rule", [&"reroll", &"score_limit"]),
		_make_def(TOOL_PROBABILITY_DOUBLER, "概率六面器", "Oops! All 6s", 126, &"epic", "所有明确写在效果里的概率翻倍，最高不超过必定成功。", &"probability_rule", [&"chance"]),
		_make_def(TOOL_IDOL_TARGET, "偶像目标器", "The Idol", 127, &"rare", "每回合指定 1 个目标点数和 1 个目标槽位特征；同时满足的被结算骰面终倍率 ×2。", &"round_start_and_score", [&"xmult", &"target"], [], [&"idol_pip", &"idol_feature"]),
		_make_def(TOOL_LOW_PLUS_OTHER_X2, "低点双视器", "Seeing Double", 128, &"rare", "若当前回合至少有 1 个低点面被结算，且至少有 1 个非低点面被结算，终倍率 ×2。", &"on_score", [&"xmult", &"low"]),
		...
		_make_def(TOOL_FOUR_KIND_X4, "四同四倍器", "The Family", 133, &"epic", "若当前回合有效被结算骰面中存在四同结构，终倍率 ×4。", &"on_score", [&"xmult", &"four_kind"]),
		...
		_make_def(TOOL_DELAYED_CLONE_SALE, "隐形复刻器", "Invisible Joker", 137, &"epic", "安装后经过 2 个结算回合进入可复刻状态；出售时随机复制 1 个其他已安装骰具为骰具道具。", &"round_end_and_sale", [&"copy", &"dice_tool"], [], [&"rounds_held"]),
		_make_def(TOOL_LEFT_COPY_BRAINSTORM, "左侧脑暴", "Brainstorm", 138, &"legendary", "复制最左侧可复制骰具的能力；复制类骰具不会被复制。", &"copy", [&"copy"]),
		...
		_make_def(TOOL_BATTLE_START_FORGE_ITEM, "开战铸件师", "Cartomancer", 142, &"rare", "选择战斗时生成 1 个随机铸骰件，进入道具槽位。", &"on_battle_start", [&"generate", &"forge_item"]),
		_make_def(TOOL_COMBO_UPGRADE_SHOP_FREE, "星象免费器", "Astronomer", 143, &"rare", "骰商铺中的所有主骰型升级件和主骰型升级包价格变为 0。", &"shop_price_rule", [&"shop", &"combo_upgrade"]),
		_make_def(TOOL_FIRST_REROLL_COMBO_UPGRADE, "焦痕升级器", "Burnt Joker", 144, &"epic", "每回合第一次重投前，根据重投前选中骰面可组成的最高主骰型，使该主骰型等级 +1。", &"before_reroll", [&"combo_level", &"reroll"], [], [&"burnt_used_this_round"]),
		...
		_make_def(TOOL_SHOP_END_ITEM_COPY, "终店复刻器", "Perkeo", 150, &"legendary", "骰商铺阶段结束时，随机复制 1 个玩家持有的铸骰件或主骰型升级件；复制物进入道具槽位。", &"shop_phase_end", [&"copy", &"item"]),
	]
...
static func get_item_pool_for_rarity(rarity: StringName = &"") -> Array:
	...
	for def in get_all_defs():
		if rarity != &"" and def.rarity != rarity:
			continue
		result.append(_item_data_from_def(def))
	return result


static func get_normal_shop_item_pool(rarity: StringName = &"", circle_number: int = 8) -> Array:
	var result := []
	for def in get_all_defs():
		if rarity != &"" and def.rarity != rarity:
			continue
		if not is_available_in_normal_shop(def.tool_id, circle_number):
			continue
		result.append(_item_data_from_def(def))
	return result


static func is_available_in_normal_shop(tool_id: StringName, circle_number: int = 8) -> bool:
	var def := get_def(tool_id)
	if def == null:
		return false
	if def.rarity == &"legendary" or def.rarity == &"epic":
		return false
	var source_type := source_type_for_tool(tool_id)
	match source_type:
		TOOL_SOURCE_SPECIAL:
			return false
		TOOL_SOURCE_BASIC:
			return true
		TOOL_SOURCE_MID:
			return circle_number >= 3
		TOOL_SOURCE_LATE:
			return circle_number >= 6
		_:
			return false


static func source_type_for_tool(tool_id: StringName) -> StringName:
	var def := get_def(tool_id)
	if def == null:
		return &""
	if NORMAL_SHOP_NEVER_SOURCE_INDICES.has(def.source_index):
		return TOOL_SOURCE_SPECIAL
	if def.rarity == &"common":
		return TOOL_SOURCE_BASIC
	if NORMAL_SHOP_MID_SOURCE_INDICES.has(def.source_index):
		return TOOL_SOURCE_MID
	return TOOL_SOURCE_LATE


static func get_weighted_pack_item_pool(pack_id: StringName) -> Array:
	var result := []
	for def in get_all_defs():
		var weight := pack_weight_for_tool(def.tool_id, pack_id)
		if weight <= 0:
			continue
		var data := _item_data_from_def(def)
		data["weight"] = weight
		result.append(data)
	return result


static func pack_weight_for_tool(tool_id: StringName, pack_id: StringName) -> int:
	var def := get_def(tool_id)
	if def == null:
		return 0
	var source_type := source_type_for_tool(tool_id)
		...
		&"pack_relic_mega":
			match source_type:
				...
				TOOL_SOURCE_LATE:
					return 40
				TOOL_SOURCE_SPECIAL:
					return 2 if MEGA_PACK_EPIC_LOW_WEIGHT_SOURCE_INDICES.has(def.source_index) else 0
				_:
					return 0
		_:
			return 0
...
static func get_special_dice_tool_item_pool(rarity: StringName = &"") -> Array:
	var result := []
	for def in get_all_defs():
		if rarity != &"" and def.rarity != rarity:
			continue
		if not NORMAL_SHOP_NEVER_SOURCE_INDICES.has(def.source_index):
			continue
		result.append(_item_data_from_def(def))
	return result


static func get_generated_dice_tool_item_pool(rarity: StringName = &"") -> Array:
	var result := []
	for def in get_all_defs():
		if rarity != &"" and def.rarity != rarity:
			continue
...
static func get_legendary_dice_tool_item_pool() -> Array:
	return get_item_pool_for_rarity(&"legendary")


static func _item_data_from_def(def: DiceToolDef) -> Dictionary:
	return {
		"id": def.tool_id,
		...
		"source_type": source_type_for_tool(def.tool_id) if def != null else &"",
